...
`outbox_event(id, workspace_id, aggregate, sequence, type, payload, occurred_at, published_at)`를
사용한다. Worker는 claim 후 consumer별 idempotency ledger에 기록한다. event ordering은 같은
aggregate sequence에서만 보장한다.

## Queue

Redis는 interactive·normal·background priority queue를 제공한다. PostgreSQL `job` row가
상태 정본이고 Redis item은 wake-up signal이다. Redis 유실 시 queued row를 재조정한다.
